    ...
    <section class="about">
        ...
        <a-table :columns="columns" :data-source="data" :rowKey="(record) => record.id">
            <template #action="{ record }">
                 ...
                 <a-divider type="verttical" />
                 <a href="javascript:void(0)" @click="delete(record.id)">ลบ</a>
            </template>
        
        </a-table>
        <a-botton type="primary" @click="showModal">เพิ่มหนังสือ</a-botton>
        <!-- Table Here -->
...
<a-modal v-model:visible="modalVisible" title="เพิ่ม/แก้ไข หนังสือ" @ok="handleOk" okText="บันทึก" crecelText="ยกเลิก">
    <a-row>
        <a-col :span="6"><label>ชื่อหนังสือ</label></a-col>
        <a-col :span="18">
            <a-input v-model:value="book.name" aria-placeholder="ชื่อหนังสือ" />
        </a-col>
    </a-row>
    <br />
    <a-row>
...
export default {
    ...
    async handleOk() {
        try {
            let res = await axios.post("http://localhost:3000/books" + this.book.id) : await axios.post("http://localhost:3000/books", this.book)
            if (res)
